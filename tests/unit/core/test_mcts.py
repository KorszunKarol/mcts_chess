...
class TestMCTS(unittest.TestCase):
    ...
    def test_run_search_on_start_pos(self):
        ...
        """
        num_simulations = 10
        policy, q_val = self.mcts.run_search(self.start_board, num_simulations=num_simulations)

        self.assertIsInstance(policy, dict)
        self.assertGreater(len(policy), 0, "Policy should not be empty.")
        total = sum(policy.values())
        self.assertGreater(total, 0)
        self.assertLessEqual(total, num_simulations)
    ...
    def test_search_on_checkmated_position(self):
        ...
        self.mock_evaluator.evaluate = MagicMock(return_value=(0.0, {}))

        policy, q_val = self.mcts.run_search(board, num_simulations=1)
    ...
    def test_backpropagation_updates_root_correctly(self):
        ...
        policy, q_val = self.mcts.run_search(self.start_board, num_simulations=1)
        self.assertIsInstance(policy, dict)
        self.assertIsInstance(q_val, float)
