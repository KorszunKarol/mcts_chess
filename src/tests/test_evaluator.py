...
class TestDualHeadEvaluator(unittest.TestCase):
    ...
    def test_evaluate_output_properties(self):
        ...
        self.assertTrue(-1.0 <= value <= 1.0, "Value from model should be in [-1, 1], as it's not a terminal node.")
    ...
    def test_illegal_move_masking(self):
        ...
        for i in range(ACTION_SPACE_SIZE):
            if i not in legal_move_indices:
                self.assertEqual(policy[i], 0.0, f"Policy for illegal move index {i} should be 0.0.")
    ...
    def test_board_mirroring_consistency(self):
        ...
        board_b = chess.Board("rnbqkbnr/pp1ppppp/8/2p5/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 1")
        ...
        with mock.patch.object(self.evaluator.model, 'predict', return_value=[mock_raw_value, mock_raw_policy]) as mock_predict:
    ...
    def test_terminal_node_evaluation(self):
        ...
        self.assertAlmostEqual(np.sum(policy), 0.0, places=5,
